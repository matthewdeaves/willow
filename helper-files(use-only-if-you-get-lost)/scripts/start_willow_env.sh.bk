...
INTERACTIVE=1
# Operation mode
## operation options include wipe, rebuild, restart, migrate, continue
OPERATION=""
...
# ENVIRONMENT VARIABLES (rewrite with host machine values if needed)
# App (Simple) container
APP_BASE_SERVICE="adapter"
APP_APPEND_SERVICE="CMS"
# Service name for the main application container
MAIN_APP_SERVICE=${APP_BASE_SERVICE}+${APP_APPEND_SERVICE} 
log_dir="docker/${MAIN_APP_SERVICE}/config/logs"
...
show_help() {
    cat << EOF
${BOLD} ${APP_BASE_SERVICE} ${APP_APPEND_SERVICE} Development Environment Setup${RESET}
...
if chmod 777 docker/logs/nginx 2>/dev/null; then
    print_success "Created docker/logs/nginx directory"
else
    print_warning "Could not set permissions on docker/logs/nginx (may need sudo)"
fi
